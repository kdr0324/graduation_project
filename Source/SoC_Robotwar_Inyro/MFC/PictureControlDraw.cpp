...
#include "PictureControlDraw.h"

// CPictureControlDraw

CPictureControlDraw::CPictureControlDraw()
{
	m_image = NULL;
}
...
bool _DrawImage(HDC hdc, const IMG_INFO *image,
								int nDstX, int nDstY, int nDstW, int nDstH)
{
	...
	int nWidth = image->m_nWidth;
	int nHeight = image->m_nHeight;
	int nBPP = (image->m_nChannels) << 3;
...
void CPictureControlDraw::Init_Image(int nWidth, int nHeight, int nChannels)
{
	if(m_image != NULL)
		ReleaseImageInfo(&m_image);
	m_image = CreateImageInfo(nWidth, nHeight, nChannels, D_U8);
}

void CPictureControlDraw::DrawToHDC()
{
	if(m_pStaticFrame.pStaticFrameDC != NULL) {
		RECT rect = m_pStaticFrame.FrameRect;
		HDC dc = m_pStaticFrame.pStaticFrameDC->m_hDC;
		_DrawImage(dc, m_image, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top);
	}
}